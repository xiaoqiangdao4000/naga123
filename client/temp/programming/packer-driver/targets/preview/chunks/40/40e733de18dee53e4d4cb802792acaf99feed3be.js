            ...
            case 'btn_ok':
              {
                ...
                var password = this.password1_editbox.string;
                var password1 = this.password1_editbox.string;

                if (nickname == '') {
                  globalThis.eventTargets.emit('chick_nickname', '用户名不能为空');
                  return;
                }

                if (password == '' || password1) {
                  globalThis.eventTargets.emit('chick_password', '密码不能为空，请输入6~10位数字或字母！');
                  break;
                }

                if (password1 != password) {
                  globalThis.eventTargets.emit('chick_password', '两次密码输入不一致，请确认！');
                  break;
                }
...
//# sourceMappingURL=40e733de18dee53e4d4cb802792acaf99feed3be.js.map